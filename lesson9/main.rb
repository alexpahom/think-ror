...
def print_station_info(station)
  ...
  station.within_trains do |train|
    puts "\tПоезд номер: #{train.number} Тип: #{train.kind} Кол-во вагонов: #{train.cars.count}"
...
def print_train_info(train)
  ...
  train.within_cars do |car|
    if car.is_a? PassengerCar
      puts "\t\tНомер:#{car.number} Тип:#{car.kind} Мест занято:#{car.seats_taken_number}" \
           " Мест свободно:#{car.vacant_seats_number}"
    elsif car.is_a? CargoCar
      puts "\t\tНомер:#{car.number} Тип:#{car.kind} Объема занято:#{car.loaded} Объема свободно:#{car.free_load}"
    end
  end
end
